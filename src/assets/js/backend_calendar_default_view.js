...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        $calendarPage.on('click', '.edit-popover', function () {
            ...
            if (lastFocusedEventData.data.is_unavailable == false) {
                ...
                // Set the start, end, and timezone data of the appointment.
				// We're now interacting with the calendar API, so moments are expressed in the local timezone.
                var startDatetime = moment.tz(appointment.start_datetime, 'UTC').tz(GlobalVariables.timezone);
				$dialog.find('#start-datetime')[0]._flatpickr.setDate(startDatetime.format('YYYY-MM-DD HH:mm'))

                var endDatetime = moment.tz(appointment.end_datetime, 'UTC').tz(GlobalVariables.timezone);
				$dialog.find('#end-datetime')[0]._flatpickr.setDate(endDatetime.format('YYYY-MM-DD HH:mm'));
				$dialog.find('#hidden-timezone').val(appointment.timezone);
                ...
                var startDatetime = moment.tz(unavailable.start_datetime, GlobalVariables.timezone);
                unavailable.end_datetime = lastFocusedEventData.end.format('YYYY-MM-DD HH:mm:ss');
                var endDatetime = moment.tz(unavailable.end_datetime, GlobalVariables.timezone);

                $dialog = $('#manage-unavailable');
                BackendCalendarUnavailabilitiesModal.resetUnavailableDialog();

                // Apply unavailable data to dialog.
				$dialog.find('#input-timezone').val(GlobalVariables.timezone);
                $dialog.find('.modal-header h3').text('Edit Unavailable Period');
                $dialog.find('#unavailable-start')[0]._flatpickr.setDate(startDatetime.format('YYYY-MM-DD HH:mm'));

                $dialog.find('#unavailable-id').val(unavailable.id);
                $dialog.find('#unavailable-provider').val(unavailable.id_users_provider);
                $dialog.find('#unavailable-end')[0]._flatpickr.setDate(endDatetime.format('YYYY-MM-DD HH:mm'));
            }
        ...
        $calendarPage.on('click', '.delete-popover', function () {
            ...
            if (lastFocusedEventData.data.is_unavailable == false) {
                var buttons = [
                    {
                        text: 'OK',
                        click: function () {
                            var url = GlobalVariables[/* @mangle */ 'baseUrl' /* @/mangle */ ] + /* @mangle */ '/index.php/backend_api/ajax_delete_appointment' /* @/mangle */;
                            var data = {
                ...
                var url = GlobalVariables[/* @mangle */ 'baseUrl' /* @/mangle */ ] + /* @mangle */ '/index.php/backend_api/ajax_delete_unavailable' /* @/mangle */;
                var data = {
                    csrfToken: GlobalVariables[/* @mangle */ 'csrfToken' /* @/mangle */ ],
    ...
    function _calendarEventResize(event, delta, revertFunc, jsEvent, ui, view) {
        ...
        if (event.data.is_unavailable == false) {
            ...
            event.data.end_datetime = moment.tz(event.data.end_datetime, 'UTC')
            ...
            var successCallback = function (response) {
                ...
                var undoFunction = function () {
                    appointment.end_datetime = event.data.end_datetime = moment.tz(
                        appointment.end_datetime, 'UTC')
                        .add({days: -delta.days(), hours: -delta.hours(), minutes: -delta.minutes()})
                        .format('YYYY-MM-DD HH:mm:ss');
            ...
            var unavailable = {
                id: event.data.id,
                end_datetime: moment.tz(event.end.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.timezone).tz('UTC').format('YYYY-MM-DD HH:mm:ss'),
            ...
            var successCallback = function (response) {
                ...
                // Display success notification to user.
                var undoFunction = function () {
                    unavailable.end_datetime = event.data.end_datetime = moment.tz(
                        unavailable.end_datetime, 'UTC')
                        .add({minutes: -delta.minutes()})
                        .format('YYYY-MM-DD HH:mm:ss');

                    var url = GlobalVariables[/* @mangle */ 'baseUrl' /* @/mangle */ ] + /* @mangle */ '/index.php/backend_api/ajax_save_unavailable' /* @/mangle */;
                    var data = {
                        csrfToken: GlobalVariables[/* @mangle */ 'csrfToken' /* @/mangle */ ],
    ...
    function _calendarEventDrop(event, delta, revertFunc, jsEvent, ui, view) {
        ...
        if (event.data.is_unavailable == false) {
            ...
            appointment.start_datetime = moment.tz(
                appointment.start_datetime, 'UTC')
                .add({days: delta.days(), hours: delta.hours(), minutes: delta.minutes()})
                .format('YYYY-MM-DD HH:mm:ss');

            appointment.end_datetime = moment.tz(
                appointment.end_datetime, 'UTC')
                .add({days: delta.days(), hours: delta.hours(), minutes: delta.minutes()})
                .format('YYYY-MM-DD HH:mm:ss');
            ...
            var unavailable = {
                id: event.data.id,
                start_datetime: moment.tz(event.start.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.timezone).tz('UTC').format('YYYY-MM-DD HH:mm:ss'),
                end_datetime: moment.tz(event.end.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.timezone).tz('UTC').format('YYYY-MM-DD HH:mm:ss'),
                id_users_provider: event.data.id_users_provider
            };
            ...
            var successCallback = function (response) {
                ...
                var undoFunction = function () {
                    unavailable.start_datetime = moment.tz(
                        unavailable.start_datetime, 'UTC')
                        .add({days: -delta.days(), minutes: -delta.minutes()})
                        .format('YYYY-MM-DD HH:mm:ss');

                    unavailable.end_datetime = moment.tz(
                        unavailable.end_datetime, 'UTC')
                        .add({days: -delta.days(), minutes: -delta.minutes()})
                        .format('YYYY-MM-DD HH:mm:ss');
    ...
    function _refreshCalendarAppointments($calendar, recordId, filterType, startDate, endDate) {
        var url = GlobalVariables[/* @mangle */ 'baseUrl' /* @/mangle */ ] + /* @mangle */ '/index.php/backend_api/ajax_get_calendar_appointments' /* @/mangle */;
        var data = {
            csrfToken: GlobalVariables[/* @mangle */ 'csrfToken' /* @/mangle */ ],
            record_id: recordId,
            start_date: startDate.clone().add(-1, 'days').format('YYYY-MM-DD'),
            end_date: endDate.clone().add(+1, 'days').format('YYYY-MM-DD'),
        ...
        $.post(url, data, function (response) {
            ...
            $.each(response.appointments, function (index, appointment) {
                var event = {
                    ...
                    start: moment.tz(appointment.start_datetime.replace(' ', "T"), 'UTC').tz(GlobalVariables.timezone),
                    end: moment.tz(appointment.end_datetime.replace(' ', "T"), 'UTC').tz(GlobalVariables.timezone),
                    allDay: false,
                    ...
                    if (provider.id == recordId) {
						...
						$.each(response.unavailables, function (index, unavailable) {
							...
							unavailablePeriod = {
								...
								start: moment.tz(unavailable.start_datetime, 'UTC').tz(GlobalVariables.timezone),
								end: moment.tz(unavailable.end_datetime, 'UTC').tz(GlobalVariables.timezone),
								allDay: false,
								color: '#879DB4',
								editable: true,
						...
						/////// MIDNIGHT TIMESTAMP OF THE FIRST CALENDAR DATE (IN LOCAL TIMEZONE).
						/////// NOTE THAT THE TIMEZONES SEEM PRECISELY BACKWARDS HERE, ABSOLUTELY COUNTERINTUITIVE
						/////// (where 'UTC' one would expect GlobalVariables.timezone, and vice-versa).
						var currentCalendarStartDate = moment.tz(
							$calendar.fullCalendar('getView').start.clone().format('YYYY-MM-DD HH:mm'), 'UTC'
						).tz(GlobalVariables.timezone);

						var index = 0;
						$.each(workingPlan, function (day, workingDay) {
							// Add a full day unavailable event. We repeat for the week before and after to cover any timezone rollovers.
							if (workingDay == null) {
								for (var i = -1; i < 2; i++) {
									unavailablePeriod = {
										title: EALang.not_working,
										start: currentCalendarStartDate.clone().add(7 * i + index, 'day'),
										end: currentCalendarStartDate.clone().add(7 * i + index + 1, 'day'),
										allDay: false,
										color: '#BEBEBE',
										editable: false,
							...
							if (workingDay != null) {
								for (var i = -1; i < 2; i++) {
									var workingDayStartMoment = currentCalendarStartDate.clone().tz('UTC').add(7 * i + index, 'day')
										.set({'hour': workingDay.start.split(':')[0], 'minute': workingDay.start.split(':')[1]}).tz(GlobalVariables.timezone);

									if (currentCalendarStartDate < workingDayStartMoment) {
										unavailablePeriod = {
											title: EALang.not_working,
											start: currentCalendarStartDate.clone().add(7 * i + index, 'days'),
											end: workingDayStartMoment,
											allDay: false,
											color: '#BEBEBE',
											editable: false,
											className: 'fc-unavailable'
							...
							if (workingDay != null) {
								for (var i = -1; i < 2; i++) {
									var workingDayEndMoment = currentCalendarStartDate.clone().tz('UTC').add(7 * i + index, 'day')
										.set({'hour': workingDay.end.split(':')[0], 'minute': workingDay.end.split(':')[1]}).tz(GlobalVariables.timezone);

									if (currentCalendarStartDate < workingDayEndMoment) {
										unavailablePeriod = {
											title: EALang.not_working,
											start: workingDayEndMoment,
											end: currentCalendarStartDate.clone().add(7 * i + index + 1, 'days'),
											allDay: false,
											color: '#BEBEBE',
											editable: false,
											className: 'fc-unavailable'
							...
							if (workingDay != null) {
								$.each(workingDay.breaks, function (i, currentBreak) {

									var unavailablePeriod = {
										title: EALang.break,
										start: currentCalendarStartDate.clone().tz('UTC').add(index, 'day')
											.set({'hour': currentBreak.start.split(':')[0], 'minute': currentBreak.start.split(':')[1]}).tz(GlobalVariables.timezone),
										end: currentCalendarStartDate.clone().tz('UTC').add(index, 'day')
											.set({'hour': currentBreak.end.split(':')[0], 'minute': currentBreak.end.split(':')[1]}).tz(GlobalVariables.timezone),
										allDay: false,
										color: '#BEBEBE',
										editable: false,
    ...
    exports.initialize = function () {
        ...
        exports.renderCalendar = function(tz) {
			$('#calendar').fullCalendar({
				...
				select: function (start, end, jsEvent, view) {
					...
					$('#insert-unavailable').trigger('click', {start: start, end: end});
		...
		exports.renderCalendar(GlobalVariables.timezone);
