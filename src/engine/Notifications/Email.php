...
class Email {
    ...
    public function sendAppointmentDetails(
		...
		$localTimezone = new \DateTimeZone($appointment['timezone']);
		$localStartDate = new \DateTime($appointment['start_datetime'], $UTCtimezone);
		$localStartDateLong = (new \Carbon\Carbon($localStartDate->setTimezone($localTimezone)->format('Y-m-d H:i'), $appointment['timezone']))->locale($this->config['language'])->isoFormat('llll');
		$localStartDateShort = (new \Carbon\Carbon($localStartDate->setTimezone($localTimezone)->format('Y-m-d H:i'), $appointment['timezone']))->locale($this->config['language'])->isoFormat('l LT');
		$localEndDate = new \DateTime($appointment['end_datetime'], $UTCtimezone);
		$localEndDate = (new \Carbon\Carbon($localEndDate->setTimezone($localTimezone)->format('Y-m-d H:i'), $appointment['timezone']))->locale($this->config['language'])->isoFormat('llll');

		$timezone_i18 = \IntlTimeZone::createTimeZone($appointment['timezone'])->getDisplayName(
			false,
			3,	// Show country name instead of timezone to circumnavigate winter/summer time questions.
        ...
        $replaceArray = [
            ...
            '$appointment_start_date' => $localStartDateLong,
			'$appointment_end_date' => $localEndDate,
			'$appointment_timezone' => $timezone_i18,
        ...
        $mailer->Subject = $title->get() . ' [' . $localStartDateShort . '] ';
        $mailer->Body = $html;
    ...
    public function sendDeleteAppointment(
		...
		$localTimezone = new \DateTimeZone($appointment['timezone']);
		$localStartDate = (new \DateTime($appointment['start_datetime']))->setTimezone($localTimezone);
        ...
        $replaceArray = [
            ...
            '$appointment_service' => $service['name'],
            '$appointment_provider' => $provider['first_name'] . ' ' . $provider['last_name'],
			'$appointment_date' => (new \Carbon\Carbon($localStartDate->setTimezone($localTimezone)->format('Y-m-d H:i'), $appointment['timezone']))->locale($this->config['language'])->isoFormat('llll'),
			'$appointment_timezone' => $timezone_i18,
