...
class Google extends CI_Controller {
     ...
     * documentation of OAuth), every TekhneeAppointments installation should use its own calendar api key. So in every
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and TekhneeAppointments.
     *
    ...
    public function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                // If current appointment has never been synced with google...
				...
				// If appointment has been synced with google...
                {
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is already cancelled; if in doubt, please contact support.');
                        }

                        $is_different = FALSE;
                        $appt_start = strtotime($appointment['start_datetime']);
						$appt_end = strtotime($appointment['end_datetime']);
						$appt_timezone = $appointment['timezone'];
						$evt_timezone = $google_event->getStart()->getTimeZone();
                        $event_start = strtotime($google_event->getStart()->getDateTime());
                        $event_end = strtotime($google_event->getEnd()->getDateTime());

                        if ($appt_start != $event_start || $appt_end != $event_end || $appt_timezone != $evt_timezone)
                        {
							$is_different = TRUE;
                        }

						// ... check whether the gcal event is different from e!a appointment, and if so update our DB records...
                        if ($is_different)
                        {
                            $appointment['start_datetime'] = (new DateTime($event_start))->format('Y-m-d H:i:s');
							$appointment['end_datetime'] = (new DateTime($event_end))->format('Y-m-d H:i:s');
							$appointment['timezone'] = $evt_timezone;
                            $this->appointments_model->add($appointment);
						}
						// ... otherwise apparently there is nothing to sync.
					}
					// If appointment has been synced with Google but is no longer available on Google, it should be deleted from our DB.
                    catch (Exception $exc)
                    {
