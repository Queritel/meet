 ...
 * and the TekhneeAppointments system. Do not place any model handling inside this
 * library.
 *
 * @package Libraries
 */
class Google_Sync {
     ...
     * where the user grants access to his data for the TekhneeAppointments app.
     */
     ...
     * when the provider granted consent to TekhneeAppointments for use his
     ...
     * Sync utility of TekhneeAppointments and the stored access token is still valid.
     ...
     * provided appointment record of TekhneeAppointments.
     *
     * @param array $appointment Contains the appointment record data.
     * @param array $provider Contains the provider record data.
     * @param array $service Contains the service record data.
    ...
    public function add_unavailable($provider, $unavailable)
    {
		...
		$start->setTimeZone($unavailable['timezone']);
		$event->setStart($start);

        $end = new Google_EventDateTime();
		$end->setDateTime(date3339(strtotime($unavailable['end_datetime'])));
		$end->setTimeZone($unavailable['timezone']);
        $event->setEnd($end);
