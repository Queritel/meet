...
class Ics_file {
    ...
    public function get_stream($appointment, $service, $provider, $customer)
    {
		...
		$appointment_start = (new DateTime($appointment['start_datetime'], $UtcTimezone))->setTimezone($localTimezone);
        $appointment_end = (new DateTime($appointment['end_datetime'], $UtcTimezone))->setTimezone($localTimezone);
        ...
        $calendar
            ->setProdId('-//TekhneeAppointments//Self-hosted appointment-booking system for round-the-clock, timezone-aware, multilingual operations. EasyAppointments fork.//EN')
            ->setTimezone(new DateTimeZone($appointment['timezone']))
            ->addEvent($event);
